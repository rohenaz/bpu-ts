import {Tx as $lWssT$Tx, OpCode as $lWssT$OpCode, Address as $lWssT$Address} from "@ts-bitcoin/core";
import $lWssT$bitcoindrpc from "bitcoind-rpc";
...
parcelRequire.register("cXVoE", function(module, exports) {
...
const fromHash = function(o, config) {
    ...
    const rpc = new (0, $lWssT$bitcoindrpc)(config);
...
const fromTx = function(o) {
    ...
    return new Promise(function(resolve, reject) {
        if (transaction) {
            const gene = (0, $lWssT$Tx).fromHex(transaction);
    ...
    xputs.forEach(function(xput, xput_index) {
        if (xput.script) {
            ...
            xput.script.chunks.forEach(function(c, chunk_index) {
                if (c.buf) {
                    ...
                    if (typeof c.opCodeNum !== "undefined") {
                        ...
                        const ops = (0, $lWssT$OpCode)[op].toString();
                        let splitter;
                        let isSplitter = false;
                        if (o.split && Array.isArray(o.split)) o.split.forEach(function(setting) {
            ...
            if (type === "in") {
                ...
                const address = (0, $lWssT$Address).fromTxInScript(xput.script).toString();
                ...
                const address = (0, $lWssT$Address).fromTxOutScript(xput.script).toString();
